...
# g <- function(Y,indic.deriv=0){
#   # Y is a row vector
#   g1 <- log.g.gaussian(Y[,1],mu = 0,sigma = 1,indic.deriv)
#   #  g2 <- log.g.gaussian(Y[,2],mu = 0,sigma = 1,indic.deriv)
#   g2 <- log.g.student(Y[,2],nu = 5,indic.deriv)
#   log.L <- cbind(g1$log.g,g2$log.g)
#   if(indic.deriv==0){
#     d.log.L <- NULL
#   }else{
#     d.log.L <- cbind(g1$d.log.g,g2$d.log.g)
#   }
...
find.permut.old <- function(C,C.permut){
  ...
  vec.1 <- matrix(1,n,1)
  score <- 100000
  C.permut.permut <- NULL
  C.permut.aux <- C.permut
  for(i in 1:n){
    diff.C <- abs(C.permut.aux) - abs(C[,i]) %*% t(vec.1)
    aux <- apply(diff.C,2,sum)
    indic.i <- which(abs(aux)==min(abs(aux)))
    best.column <- C.permut.aux[,indic.i]
    if(sum(abs(C[,i] - C.permut.aux[,indic.i]))<sum(abs(C[,i] + C.permut.aux[,indic.i]))){
      C.permut.permut <- cbind(C.permut.permut,C.permut.aux[,indic.i])
    }else{
      C.permut.permut <- cbind(C.permut.permut,-C.permut.aux[,indic.i])
    }
    C.permut.aux[,indic.i] <- 10000
  }
  ...
  for(i in 1:dim(all.permut)[3]){
    ...
    for(j in 1:dim(C.aux.diff.signs)[3]){
      ...
      if(ddiff < best.ddiff){
        ...
        best.ddiff <- ddiff
        best.permut <- all.permut[,,i]
      }
    }
  }
  return(list(C.permut = C.permut.permut,
...
# plot(x,g(x,a),type="l",col="blue",xlim=c(-1,0),ylim=c(0,1))
# lines(x,0*x,col="red")
